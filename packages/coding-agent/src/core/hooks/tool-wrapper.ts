...
import type { AgentTool, ToolExecuteOptions } from "@mariozechner/pi-ai";
...
export function wrapToolWithHooks<T>(tool: AgentTool<any, T>, hookRunner: HookRunner): AgentTool<any, T> {
	return {
		...tool,
		execute: async (toolCallId: string, params: Record<string, unknown>, options?: ToolExecuteOptions) => {
			...
			const result = await tool.execute(toolCallId, params, options);
