...
export const bashTool: AgentTool<typeof bashSchema, BashDetails> = {
	...
	execute: async (
		toolCallId,
		{ command, workdir, env, yieldMs, stdinMode },
		{ signal, emitEvent, yieldSignal } = {},
	) => {
		if (!command) {
						...
						content: [
							{
								type: "text",
								text:
									`Command still running (session ${sessionIdNew}, pid ${child.pid ?? "n/a"}). ` +
